...
public:
  ...
  Wrench(const Frame &frame = Frame::kWorld,
         const Eigen::VectorXd &wrench = Eigen::VectorXd::Zero(6));
  Wrench(const Frame &frame, const Eigen::Vector3d &force, const Eigen::Vector3d &torque);
  ~Wrench() = default;

  const Frame &getFrame() const;

  const Eigen::VectorXd &getOriginWrench() const;

  Eigen::Vector3d getOriginForce() const;

  Eigen::Vector3d getOriginTorque() const;
  ...
  /**
   * @fn computeOriginWrenchFromPointWrench
   * @brief Calculate Ta = Pab x Fb
   *
   * @param translation_to_point_in_world_frame
   * @return Wrench
   */
  Wrench computeOriginWrenchFromPointWrench(
